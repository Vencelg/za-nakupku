                                ...
                                <table class="min-w-full">
                                    ...
                                    @foreach($payments as $payment)
                                        <tr class="border-b hover:bg-green-50 transition-all">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                                <a href="{{ route('user.detail', ['id' => $payment->user_id]) }}"
                                                   class="text-green-500
                                        hover:text-green-600">{{$payment->user_id}}</a>
                                            </td>
                                            <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                                                <a href="{{ route('listing.detail', ['id' => $payment->listing_id]) }}"
                                                   class="text-green-500
                                        hover:text-green-600">{{$payment->listing_id}}</a>
                                            </td>
                                            <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                                                {{$payment->amount}}
                                            </td>
                                            ...
                                            <td class="text-sm text-gray-900 font-light whitespace-nowrap">
                                                <a href="{{route('payment.detail', ['id' => $payment->id])}}">
                                                    <button type="button"
